...
('Fish curry', 'Enjoy a variety of homemade fish dishes.', 'Bangladesh', '2025-05-10 19:00:00', 10, 50.00, '2025-05-4');
...
UPDATE Meal
SET title = 'Italian Italian Italian',
    price = 28.50
WHERE id = 1;
...
(4, 2, '2025-06-10', '123-456-789000', 'niger ', 'ali@example.com');
...
UPDATE `Reservation`
set contact_email = "aliiiiii@mail.com"
WHERE id = 4;
...
('Amazing boiled egg', 'The boiled egg was cooked to perfection.', 4, 5, '2025-06-20');
...
select CAST(SUM(number_of_guests) AS UNSIGNED)  as number_of_guests, cast(`Meal`.max_reservations AS UNSIGNED) as max_reservations, meal_id,  `Meal`.title  from `Reservation`
LEFT JOIN `Meal` on `Meal`.id = `Reservation`.meal_id 
GROUP BY meal_id
HAVING number_of_guests < max_reservations;
...
SELECT * from `Meal`
where created_date BETWEEN '2024-06-01' and '2024-06-02';
